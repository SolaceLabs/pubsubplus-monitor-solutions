...
/**
 * Class to retrieve all queue details from a service. 
 * @author VictorTsonkov
 *
 */
public class ServiceQueueDetailsTask extends McTask
{
	...
	private static final Logger logger = LoggerFactory.getLogger(ServiceQueueDetailsTask.class);
	
	/**
	 * Initialises a new instance of the class.
	 * @throws SAXException 
	 */
	public ServiceQueueDetailsTask(PubSubBroker broker, EventMonitor eventMonitor) throws SAXException
	{
		if (broker == null)
	...
	public void run()
	{
		...
		try
		{
			...
			Queues queues = show.getVpnQueueDetails(broker.getVpnName());
			if (enrichedFormat)
				eventData = new SplunkEventData(splunkSourceType, splunkIndex, new EventData(broker.getServiceName(), broker.getVpnName(), "QueueDetail", queues));
			else
		...
		{
			logger.error("Error occured while obtaining queue details for service: {}, exception: {}, stack: {}", broker.getServiceName(), e.getMessage(), e.getStackTrace());
		}
	}
}
