...
ds=$(date "+%Y/%m/%d-%H.%M.%S")
echo "$ds CLEAR $* ($# args)" >> $PSMON_OUT_FILE
[ "$VERBOSE" == "-v" ] && echo "Running < $SNOW_POST $VERBOSE --clear --argv $* >" >> $PSMON_OUT_FILE

cd $SNOW_HOME
$SNOW_POST $VERBOSE --dir $SNOW_HOME --clear --argv $* >> $PSMON_OUT_FILE 2>&1
