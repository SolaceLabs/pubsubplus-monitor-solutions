...
my $np = Nagios::Plugin->new(
    usage => "Usage: %s  -r|--router <router mgmt ip> -u|--user <username> -p|--pass <password> [--wt|--warnTransSess <usage %> --ct|--critTransSess <usage %>]
...
my $rpccallShow = "<rpc semp-version=\"soltr/5_3\"><show><message-spool><detail/></message-spool></show></rpc>";
my $ua;
my $req;
my $res;
...
if($dom->findvalue("/rpc-reply/rpc/show/message-spool/message-spool-info/operational-status") eq MSGSPOOLOPSTATE_ADSTANDBY){
	if ($np->opts->quiet){
			$np->nagios_exit(OK,"$host is ".MSGSPOOLOPSTATE_ADSTANDBY);
	}else{
			$np->nagios_exit(CRITICAL,"$host is ".MSGSPOOLOPSTATE_ADSTANDBY." use --quiet option to get rid of the error");
	}
}
my @states;
my @messages;
